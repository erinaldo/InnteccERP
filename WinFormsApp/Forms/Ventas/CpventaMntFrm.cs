...
namespace WinFormsApp
{
    public partial class CpventaMntFrm : XtraForm
    {
        ...
        public CpventaMntFrm(int idEntidadMnt, TipoMantenimiento tipoMnt, GridControl gridParent, CpventaFrm formParent, int idPacienteARegistrar, int? idprogramacioncitadet, int? idMotivoCita)
        {
            if (tipoMnt == TipoMantenimiento.SinEspecificar && idEntidadMnt <= 0)
            {
                throw new ArgumentException("El valor primario de la entidad no contiene un valor valido.");
        ...
        private void CargarSocioNegocioPaciente()
        {
            ...
            if (IdPacienteARegistrar > 0)
            {
                ...
                CargarArticuloMotivo();
            }
        }

        private void CargarArticuloMotivo()
        {
            int idSucursal = 0;
            int idAlmacen = 0;
            ...
            {
                idSucursal = VwSucursalSel.Idsucursal;
                idAlmacen = VwSucursalSel.Idalmacendefecto;
            }

            int idArticulo = Convert.ToInt32(IdMotivoCita);

            int idTipoLista = (int) rIdtipolista.EditValue;

            VwArticulostocklista vwArticulolistaprecio = Service.ConsultaArticuloStockLista(idSucursal, idAlmacen, idTipoLista, SessionApp.DateServer, idArticulo).FirstOrDefault();
            if (vwArticulolistaprecio == null)
            {
                return;
            }

            int? idAreaEmpleado = null, idProyectoCliente = null, idCentroBeneficio = null;
            VwEmpleado vwEmpleadoSel =
                CacheObjects.VwEmpleadoList.FirstOrDefault(x => x.Idempleado == (int) iIdvendedor.EditValue);
            VwSocionegocioproyecto vwSocionegocioproyectoDefecto =
                CacheObjects.VwSocionegocioproyectoList.FirstOrDefault(
                    x => x.Idsocionegocio == (int) iIdcliente.EditValue && x.Proyectopordefecto);
            if (vwEmpleadoSel != null && vwSocionegocioproyectoDefecto != null)
            {
                idAreaEmpleado = vwEmpleadoSel.Idarea;
                idProyectoCliente = vwSocionegocioproyectoDefecto.Idproyecto;
                idCentroBeneficio = SessionApp.SucursalSel.Idcentrobeneficioventadefecto;
            }

            int? idUbicacionPorDefecto = null;
            Almacen almacen = Service.GetAlmacen(idAlmacen);
            if (almacen != null)
            {
                idUbicacionPorDefecto = almacen.Idubicaciondefecto;
            }

            VwCpventadet vwCpventadetMnt = new VwCpventadet();
            vwCpventadetMnt.Numeroitem = 1;
            vwCpventadetMnt.Idarticulo = Convert.ToInt32(vwArticulolistaprecio.Idarticulo);
            vwCpventadetMnt.Codigoarticulo = vwArticulolistaprecio.Codigoarticulo;
            vwCpventadetMnt.Codigoproveedor = vwArticulolistaprecio.Codigoproveedor;
            vwCpventadetMnt.Idunidadmedida = Convert.ToInt32(vwArticulolistaprecio.Idunidadmedida);
            vwCpventadetMnt.Abrunidadmedida = vwArticulolistaprecio.Abrunidadmedida;
            vwCpventadetMnt.Nombremarca = vwArticulolistaprecio.Nombremarca;
            vwCpventadetMnt.Nombrearticulo = vwArticulolistaprecio.Nombrearticulo;
            vwCpventadetMnt.Cantidad = 1;
            vwCpventadetMnt.Abrunidadmedida = vwArticulolistaprecio.Abrunidadmedida;
            vwCpventadetMnt.Preciounitario = vwArticulolistaprecio.Preciocontado;
            vwCpventadetMnt.Especificacion = string.Empty;
            vwCpventadetMnt.Descuento1 = 0m;
            vwCpventadetMnt.Descuento2 = 0m;
            vwCpventadetMnt.Descuento3 = 0m;
            vwCpventadetMnt.Descuento4 = 0m;
            vwCpventadetMnt.Preciounitarioneto = vwArticulolistaprecio.Preciocontado;
            vwCpventadetMnt.Importetotal = vwArticulolistaprecio.Preciocontado;
            vwCpventadetMnt.Idimpuesto = Convert.ToInt32(vwArticulolistaprecio.Idimpuesto);
            vwCpventadetMnt.Idalmacen = idAlmacen;
            vwCpventadetMnt.Idtipoafectacionigv = Convert.ToInt32(vwArticulolistaprecio.Idtipoafectacionigv);
            vwCpventadetMnt.Gravado = vwArticulolistaprecio.Gravado;
            vwCpventadetMnt.Exonerado = vwArticulolistaprecio.Exonerado;
            vwCpventadetMnt.Inafecto = vwArticulolistaprecio.Inafecto;
            vwCpventadetMnt.Exportacion = vwArticulolistaprecio.Exportacion;
            vwCpventadetMnt.Idarea = Convert.ToInt32(idAreaEmpleado);
            vwCpventadetMnt.Nombrearea = string.Empty;
            vwCpventadetMnt.Idproyecto = Convert.ToInt32(idProyectoCliente);
            vwCpventadetMnt.Nombreproyecto = string.Empty;
            vwCpventadetMnt.Idcentrodecosto = Convert.ToInt32(idCentroBeneficio);
            vwCpventadetMnt.Descripcioncentrodecosto = string.Empty;
            vwCpventadetMnt.Porcentajepercepcion = vwArticulolistaprecio.Aplicapercepcion
                ? SessionApp.EmpresaSel.Porcentajepercepcion
                : 0m;
            vwCpventadetMnt.Idubicacion = Convert.ToInt32(idUbicacionPorDefecto);

            //Los items por defecto se calculan
            vwCpventadetMnt.Calcularitem = true;
            vwCpventadetMnt.Bonificacion = false;
        ...
        private void EstadoReferenciaCaja()
        {
            ...
            if (Service.CpVentaTieneReferenciaCaja(idTipoCp, serieTipoCp, numeroTipoCp))
            {
                ...
                HabilitarBotonesMnt(false);
                CamposSoloLectura(true);
                btnReciboIngreso.Enabled = false;
                gvDetalle.OptionsBehavior.Editable = false;
            }
        }
        ...
        private bool Guardar()
        {
            ...
            try
            {
                ...
                //if (iIdvendedor.EditValue != null)
                //{
                //    CajaCobroCpVentaFrmResumen cajaCobroCpVentaFrmResumen = new CajaCobroCpVentaFrmResumen(Convert.ToInt32(IdEntidadMnt), (int)iIdvendedor.EditValue);
                //    cajaCobroCpVentaFrmResumen.ShowDialog();
                //    if (cajaCobroCpVentaFrmResumen.DialogResult == DialogResult.OK)
                //    {
        ...
        private void bmMantenimiento_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            switch (e.Item.Name)
            {
                ...
                case "btnImprimir":
                    ...
                    if (IdEntidadMnt > 0)
                    {
                        Cpventa vpCpventa = Service.GetCpventa(IdEntidadMnt);
                        ImpresionFormato.FormatoCpVenta(vpCpventa);
                    }
                    break;
                case "btnOrdenventa":
                ...
                case "btnReciboIngreso":
                    if (IdEntidadMnt == 0)
                    {
                        WinFormUtils.MessageWarning("Grabe el comprobante.");
                        return;
                    }
                    ...
                    if (iIdvendedor.EditValue != null)
                    {
                        CajaCobroCpVentaFrmResumen cajaCobroCpVentaFrm = new CajaCobroCpVentaFrmResumen(Convert.ToInt32(IdEntidadMnt), (int)iIdvendedor.EditValue);
        ...
        private void EstadoBuscadorSocioNegocio()
        {
            ...
            beSocioNegocio.Properties.Buttons[0].Enabled = true;
            beSocioNegocio.Properties.Buttons[1].Enabled = true;
        }
        ...
        private void HabilitarBotonesMnt(bool enabled)
        {
            ...
            btnReciboIngreso.Enabled = !enabled;
